$(document).ready(function() {
    setTimeout(function() {
            $.fn.dataTable.Api.register("column().data().sum()",
                function() {
            ...
            $('<button class="btn  btn-primary  m-b-20">sum of age in all rows</button>')
                ...
                .on("click", function() { alert("Column sum is: " + table.column(3).data().sum()); });
            $('<button class="btn  btn-primary m-r-10 m-b-20">sum of  age of visible rows</button>')
                .prependTo(".dt-plugin-buttons").on("click",
                    function() { alert("Column sum is: " + table.column(3, { page: "current" }).data().sum()); });
            ...
            $.fn.dataTable.ext.order["dom-text"] = function(settings, col) {
                return this.api().column(col, { order: "index" }).nodes()
                    .map(function(td, i) { return $("input", td).val(); });
            };
            ...
            $.fn.dataTable.ext.order["dom-select"] = function(settings, col) {
                return this.api().column(col, { order: "index" }).nodes()
                    .map(function(td, i) { return $("select", td).val(); });
            };
            ...
            $(document).ready(function() {
                $("#dt-live-dom").DataTable({
                    "columns": [
                        null, { "orderDataType": "dom-text-numeric" }, { "orderDataType": "dom-text", type: "string" },
            ...
            $("#dt-server-processing").DataTable({
                ...
                "columns": [
                    { "data": "first_name" }, { "data": "last_name" }, { "data": "position" }, { "data": "office" },
            ...
            $("#dt-http").DataTable({
                ...
                "ajax": {
                    ...
                    data: function(d) { d.myKey = "myValue"; }
                },
                "columns": [
                    { "data": "first_name" }, { "data": "last_name" }, { "data": "position" }, { "data": "office" },
                    { "data": "start_date" }, { "data": "salary" }
            ...
            $("#dt-ajax-object").DataTable({
                ...
                "columns": [
                    { "data": "name" }, { "data": "position" }, { "data": "office" }, { "data": "extn" },
                    { "data": "start_date" }, { "data": "salary" }
                ]
            });
            $("#dt-nested-object").DataTable({
                ...
                "columns": [
                    { "data": "name" }, { "data": "hr.position" }, { "data": "contact.0" }, { "data": "contact.1" },
                    { "data": "hr.start_date" }, { "data": "hr.salary" }
                ]
            });
            $("#dt-orthogonal").DataTable({
                ...
                columns: [
                    { data: "name" }, { data: "position" }, { data: "office" }, { data: "extn" },
                    { data: { _: "start_date.display", sort: "start_date.timestamp" } }, { data: "salary" }
                ]
            });
            ...
            $("#dt-generate-content tbody").on("click",
                "button",
                function() {
            ...
            $("#jsource-table").DataTable({
                data: dataSet,
                columns: [
                    { title: "Name" }, { title: "Position" }, { title: "Office" }, { title: "Extn." },
                    { title: "Start date" }, { title: "Salary" }
                ]
            });
            $("#server-table")
                .DataTable({ "processing": true, "serverSide": true, "ajax": "dt-json-data/server-table.php" });
            ...
            function filterGlobal() {
                $("#search-api").DataTable().search($("#global_filter").val(),
                    $("#global_regex").prop("checked"),
                    $("#global_smart").prop("checked")).draw();
            }

            function filterColumn(i) {
                $("#search-api").DataTable().column(i).search($("#col" + i + "_filter").val(),
            ...
            $("input.column_filter")
                .on("keyup click", function() { filterColumn($(this).parents("tr").attr("data-column")); });
            ...
            $("#multi-colum-dt").DataTable({
                columnDefs: [
                    { targets: [0], orderData: [0, 1] }, { targets: [1], orderData: [1, 0] },
            ...
            $("#colum-rendr").DataTable({
                "columnDefs": [
                    { "render": function(data, type, row) { return data + " (" + row[3] + ")"; }, "targets": 0 },
